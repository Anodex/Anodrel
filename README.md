...
Electron replacement. Windows release foundations include signed catalogue discovery, private
image staging, native consent, elevation handoff, post-update policy proof, and a verified
Start-menu product-launch route. Production identity, signed end-to-end acceptance, and
non-Windows desktop hosts remain planned. Linux has authenticated local transport, a host-owned
private child transport, bounded state, and a fixed Wayland child/view diagnostic—not an application desktop host or product launcher.
...
path remains unvalidated in practice**. The owned release foundations do not remove the remaining production-identity, elevation-handoff, and recovery decisions; see `ROADMAP.md` and `docs/PRODUCT_FIXTURE.md`.
...
`docs/LINUX_TRANSPORT.md`, `docs/LINUX_SESSIONS.md`, `docs/LINUX_WINDOWING.md`, and `docs/LINUX_WINDOW_SESSIONS.md` define Linux transport, development session, and fixed Wayland diagnostic boundaries.
...
`docs/RENDERER.md` documents the renderer and brand API; `docs/FONTS.md` defines the owned Unicode-to-glyph parsing foundation, `docs/TEXT_RUNS.md` the bounded glyph-run layer, and `docs/GLYPH_RENDERING.md` the path-to-canvas adapter for a future native text path.
