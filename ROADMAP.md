...
- Draw first-party surfaces with a software renderer. **Windows complete; owned text foundation started:** a portable
  ...
  owned Unicode mapping, bounded horizontal metrics and conventional pair kerning, simple-outline extraction, exact quadratic paths, bounded device-explicit flattening, one bounded coverage mask, a bounded source-order glyph run, and a face-local bounded cache for a later Linux glyph source (Decisions 0133 through 0138 and 0204 through 0208). See `docs/RENDERER.md`, `docs/FONTS.md`, `docs/TEXT_RUNS.md`, `docs/GLYPH_RENDERING.md`, and `docs/GLYPH_CACHE.md`.
