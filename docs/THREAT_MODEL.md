...
| A future application-state write exposes a partial, substituted, or cross-application value. | Keep the first state store to one bounded opaque snapshot per host-validated identity below the host-owned data location; stage and flush a complete replacement before direct atomic replacement, retain a bounded recovery candidate, and expose no path, temporary name, content, or native detail. Public access remains blocked until separate read, replace, and clear grants are documented. |
