...
| A selected path becomes arbitrary-file access or resolves to a replacement file. | A future file reader accepts only an unguessable reference created by the host for the same authenticated session, never a caller-supplied path. The Windows adapter must retain and verify native read-only file identity across replacement, rename, and reparse-point races before it can expose bytes. |
