...
| A Linux process from another UID, or one that knows an abstract endpoint name, connects to an Anodrel transport. | Create one random abstract `AF_UNIX` endpoint from `/dev/urandom`, accept one stream only, require Linux `SO_PEERCRED` to report the host's effective UID, then require a distinct 32-byte token as the first framed authentication control. The endpoint has no filesystem entry or TCP address; failed UID, handshake, frame, or I/O checks close it without native detail. See Decision 0122 and `docs/LINUX_TRANSPORT.md`. |
| A Linux child or parent redirects a host data root through inherited environment or working-directory state. | Resolve only the process effective account through direct getpwuid_r, derive the fixed default local-data root below its absolute home, and reject unavailable or malformed values. Do not read HOME, XDG variables, the working directory, application input, or a configuration file. The lookup creates and exposes no directory. See Decision 0124 and `docs/PATHS.md`. |
| A session invitation reaches another process or durable diagnostic. | Deliver one bounded record only through private child standard input; never put it in arguments, environment, logs, telemetry, crash data, or files. ANBI remains a Windows named-pipe record and ANLI remains a Linux abstract-socket record, so neither accepts the other endpoint form. The Linux child adapter opens only the sealed ANLI endpoint, while the server separately requires same-UID peer credentials and the first token control. See Decisions 0009, 0122, and 0123. |
...
| A context popup becomes a browser-data, pointer-telemetry, native-window, or callback bridge. | Protocol 1.32 has a distinct `menu.context.write` grant and accepts only one bounded complete set of semantic IDs, labels, and enabled flags. The host chooses whether to show it and derives placement only from a pointer-originated local `WM_CONTEXTMENU` for the session's primary view. No coordinate, target element, selection, link, URL, menu handle, command ID, callback, keyboard state, menu opening/dismissal, or result readback crosses the boundary. The UI thread owns the popup and private mapping; an enabled choice enters the fixed interaction mailbox as a revision-bound candidate, and `ui.events.read` later revalidates it before returning only an action ID and opaque context-menu revision. See Decision 0120 and `docs/CONTEXT_MENUS.md`. |
| A tray model creates persistent desktop authority, duplicates an icon, exposes a callback or pointer location, or delivers a stale command. | Protocol 1.33 requires a separate `tray.write` grant and accepts one to sixteen semantic ID, label, and enabled-state items. The Windows host lazily creates one host-selected Shell32 entry shared with notifications, enables only its private callback message, and accepts only local left or right button releases while a current tray exists. Right-click reads the cursor only inside the callback, uses a transient User32 popup with private commands, and destroys it before returning; left-click makes only a best-effort foreground request for that same host window. No icon, tooltip, callback value, pointer, handle, command number, focus result, click data, dismissal, or background lifetime crosses the protocol. A selected action carries a tray revision and is revalidated against the current enabled model by `ui.events.read`; stale and disabled actions are discarded. See Decision 0191 and `docs/TRAY.md`. |
