...
The accepted but not yet implemented Protocol 1.18 operation is
`menu.replace`.
...
| A menu model gains native window, shell, or command authority; a stale command is delivered; or menu interaction creates an unbounded event path. | Planned Protocol 1.18 `menu.replace` will require its own `menu.write` grant and accept only a bounded complete menu model of labels, enabled semantic IDs, and no native command number, target, shortcut, callback, payload, system-menu, context-menu, or submenu field. The Windows UI thread will own and map every private command ID. It will accept only the documented normal-menu `WM_COMMAND` shape and only a current mapped ID, then place a menu-revision candidate into the existing bounded ordered interaction mailbox. The existing `ui.events.read` grant will revalidate that revision and enabled command before returning a `menu.action.invoked` event; disabled, replaced, or removed commands are discarded. |
