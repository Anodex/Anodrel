...
Protocol 1.18 implements the portable `menu.replace` boundary. Its direct
Windows menu attachment and activation delivery remain pending.
...
| A menu model gains native window, shell, or command authority; a stale command is delivered; or menu interaction creates an unbounded event path. | Protocol 1.18 `menu.replace` requires its own `menu.write` grant and validates only a bounded complete model of labels and enabled semantic IDs — never a native command number, target, shortcut, callback, payload, system-menu, context-menu, or submenu field. Its core state commits only after the host menu service accepts the complete replacement; an unattached service returns `menu.unavailable`. The pending Windows UI thread will own and map every private command ID, accept only the documented normal-menu `WM_COMMAND` shape and a current mapped ID, then place a menu-revision candidate into the existing bounded ordered interaction mailbox. The existing `ui.events.read` grant will revalidate that revision and enabled command before returning a `menu.action.invoked` event; disabled, replaced, or removed commands are discarded. |
