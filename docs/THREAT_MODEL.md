...
| A selected folder path becomes arbitrary directory access, or a selected folder is replaced before a later enumeration. | Keep Protocol 1.28 `dialog.open_folder` display-only. The separately specified Protocol 1.29 companion may create a one-use `FolderReference` only after a host adapter captures a non-reparse directory identity and protected handle. A distinct `folder.read_entries` grant may consume it for at most 32 immediate entry names and safe kinds; it accepts no path, traversal, child selector, cursor, or write request. Before any private path-based enumeration, the adapter must take and compare a protected handle to the captured identity; a mismatch, reparse point, unknown reference, or native failure becomes only `folder.unavailable`. |
