...
`file.read_text`, `file.write_text`,
...
Protocol 1.19 reserves the host-authorized HTTPS text-fetch contract in
`docs/NETWORK.md`; it is not implemented or granted by any current host.
...
| A network request probes arbitrary, local, redirected, credentialed, or browser-state destinations; leaks response data or OS diagnostics; or leaves native network handles behind. | Reserve `network.fetch_text` for one explicit `network.fetch` grant and an exact host-selected HTTPS-origin set. Reject user information, IP literals, fragments, local origins, headers, method, body, cookies, redirects, proxy discovery, automatic authentication, client certificates, and application-selected policy before any direct WinHTTP call. Use fixed phase timeouts, normal TLS validation plus revocation checking, bounded UTF-8 text only, and RAII for every WinHTTP handle. An origin mismatch or native failure maps only to `network.unavailable`; response bytes, URL, headers, certificate, address, proxy, timing, and native status never enter diagnostics. See `docs/NETWORK.md` and Decision 0084. |
...
| A menu model gains native window, shell, or command authority; a stale command is delivered; or menu interaction creates an unbounded event path. | Protocol 1.18 `menu.replace` requires its own `menu.write` grant and validates only a bounded complete model of labels and enabled semantic IDs — never a native command number, target, shortcut, callback, payload, system-menu, context-menu, or submenu field. Its core state commits only after the host menu service accepts the complete replacement; an unattached service returns `menu.unavailable`. The Windows UI thread owns and maps every private command ID, accepts only the documented normal-menu `WM_COMMAND` shape and a current mapped ID, then places a menu-revision candidate into the existing bounded ordered interaction mailbox. The existing `ui.events.read` grant revalidates that revision and enabled command before returning a `menu.action.invoked` event; disabled, replaced, or removed commands are discarded. |
...
| An application manipulates another window, learns host window state, or turns a presentation request into arbitrary native-window control. | `window.state.set` accepts only `minimized`, `maximized`, or `restored`, with an exact one-field payload and a separate `window.state` grant at protocol 1.16. The request names no target, handle, identifier, geometry, focus, or native command; the host resolves only the authenticated session's own window and applies it from that window's UI thread. It returns acceptance only, never current/resulting state, bounds, monitor, visibility, z-order, or an event, so it cannot become host-topology readback. A one-request five-second bridge keeps an unavailable UI surface from making workers pile up or leaving the session permanently busy. See `docs/WINDOW_STATE.md` and Decision 0072. |
