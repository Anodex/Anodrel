...
`network.fetch` grant; no template, product fixture, or installed application
session receives the service.
...
| An automation caller scrolls a different or stale viewport, turns a provider into input authority, or reports a person's position to an application. | Decision 0097 exposes `IScrollProvider` only for the current first visible overflowing scroll group, carrying its semantic ID and immutable provider revision through one 250 ms host-owned request slot. A payload-free private wake message carries no pointer, position, handle, document, or application data. The owner UI thread takes the request once, rechecks the same revision and first-overflowing metric, and changes only the existing retained scroll state. A timeout clears the exact slot before a late completion can change it. There is no protocol operation, grant, event, focus move, semantic action, position readback, automation-presence signal, horizontal target, nested target, or application callback. See `docs/UI_AUTOMATION_SCROLL.md`. |
