...
| Updates | Foundation in progress | A current signed candidate must match the selected installed publisher and be strictly newer; a no-argument transaction refreshes that decision before it reuses the checked staging, promotion, and policy-publication path. No delivery or downloader is shipped. | Define delivery metadata/source, key rotation, policy-backed rollback/recovery, and user-visible update policy after distribution identity is chosen. |
