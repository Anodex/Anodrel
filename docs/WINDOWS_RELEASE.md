...
| Signed distribution and installation | Foundation in progress | The owned installer validates strict release manifests, fixed current-image payload bundles, and records the host validator accepts, without adding an installer framework. | Choose certificate custody, implement self-signature verification, staged installation, recovery, uninstall, and release verification paths. |
