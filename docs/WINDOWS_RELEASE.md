...
| Signed distribution and installation | Foundation in progress | The owned installer now validates strict release manifests and renders records the host validator accepts, without adding an installer framework. | Choose certificate custody, implement the owned bundle, installation, recovery, uninstall, and release verification paths. |
