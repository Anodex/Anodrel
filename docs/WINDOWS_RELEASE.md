...
| Signed distribution and installation | Foundation in progress | The owned tools validate strict release manifests, resource-bearing payload bundles, current-image Authenticode, private staged extraction, extracted-executable signer matching, no-overwrite promotion, fixed machine-policy publication, recovery, uninstall, and fixed direct-elevation commands without an installer framework. | Choose certificate custody, prove the signed positive path, and complete release verification/recovery proof. |
| Updates | Foundation in progress | A current signed candidate must match the selected installed publisher and be strictly newer; a no-argument transaction refreshes that decision, retains one fixed prior policy record, then reuses the checked staging, promotion, and policy-publication path. No delivery or downloader is shipped. | Define delivery metadata/source, key rotation, a rollback command and recovery proof, and user-visible update policy after distribution identity is chosen. |
