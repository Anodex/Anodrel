...
| Signed distribution and installation | Foundation in progress | The owned tools validate strict release manifests, resource-bearing payload bundles, the current Authenticode signer, and records the host validator accepts, without an installer framework. | Choose certificate custody, sign a resource-bearing image, implement staged installation, recovery, uninstall, and release verification paths. |
