...
| Updates | Foundation in progress | A current signed candidate must match the selected installed publisher and be strictly newer; a no-argument transaction refreshes that decision, retains one fixed prior record, and a separately verified fixed command can restore it. No delivery or downloader is shipped. | Define delivery metadata/source, key rotation, recovery proof, and user-visible update policy after distribution identity is chosen. |
