...
The estimate is currently **about 70%** of that Windows release goal. The host,
...
| Performance | Guarded | The release-only frame guard currently measures 6.43 ms average and 8.04 ms worst sustained frame against a 16 ms interval. | Keep the release guard, startup report, and equivalent real-application measurements in each release candidate. |
