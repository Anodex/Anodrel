...
| Accessibility | In progress | The UI Automation provider supports reading, navigation, hit testing, focus, Invoke, Value, structure, live-status, and scroll boundaries. On 2026-09-19, all seven fixed direct acceptance probes passed against the release build, including a fixed authenticated window-controls session that verifies composed title, size, maximise/restore, fullscreen/windowed restoration, and completion. | Complete every documented manual Narrator and Inspect check for the current provider, then add further patterns only when a real UI requires them. |
| Performance | Guarded | On 2026-09-19, the reference frame guard measured 6.317 ms average and 8.030 ms worst sustained frame against a 16 ms interval. The same local run reported 1.752 ms startup checks, 0.208202% CPU during one 30.018-second static-window sample, and all seven direct UI Automation probes passing. | Keep the frame guard, startup report, idle report, and equivalent real-application measurements when host or rendering behaviour changes. |
...
- the seven fixed direct UI Automation probes through
...
run the seven fixed direct Windows UI Automation probes after the non-interactive
