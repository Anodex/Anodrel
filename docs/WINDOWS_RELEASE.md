...
| Signed distribution and installation | Foundation in progress | The owned installer validates strict release manifests, fixed current-image payload bundles, its own Authenticode signer, and records the host validator accepts, without adding an installer framework. | Choose certificate custody, build a signed resource-bearing image, implement staged installation, recovery, uninstall, and release verification paths. |
