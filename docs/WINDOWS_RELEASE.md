...
| Performance | Guarded | On 2026-09-19, the reference frame guard measured 6.634 ms average and 8.436 ms worst sustained frame against a 16 ms interval. The same local run reported 1.874 ms startup checks, 1.093142% CPU during one 30.016-second static-window sample, and all eight direct UI Automation probes passing. | Keep the frame guard, startup report, idle report, and equivalent real-application measurements when host or rendering behaviour changes. |
