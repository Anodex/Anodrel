...
| Updates | Foundation in progress | A current signed candidate must match the selected installed publisher and be strictly newer; a no-argument transaction refreshes that decision, retains one fixed prior record, and a separately verified fixed command can restore it. A bounded portable catalogue now binds one later candidate image, but no signature, delivery, downloader, or automatic update is shipped. | Add exact-publisher catalogue signing, then define delivery source, key rotation, recovery proof, and user-visible update policy after distribution identity is chosen. |
