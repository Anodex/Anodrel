...
| Signed distribution and installation | Foundation in progress | The owned tools validate strict release manifests, resource-bearing payload bundles, current-image Authenticode, private staged extraction, extracted-executable signer matching, no-overwrite promotion, fixed machine-policy publication, recovery, and uninstall boundaries without an installer framework. | Choose certificate custody, prove the signed positive path, compose the bounded elevated commands, and complete release verification/recovery proof. |
| Updates | Foundation started | A read-only gate requires a current signed candidate to match the selected installed publisher and be strictly newer than the canonical installed version; no updater or download is shipped. | Define delivery metadata/source, key rotation, transactional install composition, rollback/recovery, and user-visible update policy after distribution identity is chosen. |
