...
| Performance | Guarded | On 2026-09-19, the reference frame guard measured 6.303 ms average and 8.073 ms worst sustained frame against a 16 ms interval. The same local run reported 1.614 ms startup checks, 0.000% CPU during one 30.020-second static-window sample, and all six direct UI Automation probes passing. | Keep the frame guard, startup report, idle report, and equivalent real-application measurements when host or rendering behaviour changes. |
