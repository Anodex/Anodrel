...
| Signed distribution and installation | Foundation in progress | The owned tools validate strict release manifests, resource-bearing payload bundles, current-image Authenticode, private staged extraction, extracted-executable signer matching, and same-volume no-overwrite promotion, without an installer framework. | Choose certificate custody, prove the signed positive path, then implement registry publication, recovery, uninstall, and release verification paths. |
