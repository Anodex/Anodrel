...
| Performance | Guarded | On 2026-09-19, the reference frame guard measured 6.354 ms average and 8.058 ms worst sustained frame against a 16 ms interval. The same local run reported 1.655 ms startup checks, 0.052091% CPU during one 29.995-second static-window sample, and all eight direct UI Automation probes passing. | Keep the frame guard, startup report, idle report, and equivalent real-application measurements when host or rendering behaviour changes. |
