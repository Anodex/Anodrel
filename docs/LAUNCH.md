...
| `capabilities` | Required only in 1.1. Exact non-duplicate supported grants selected by machine policy; 1.1 supports `diagnostics.read`, `ui.document.write`, `ui.events.read`, `session.close`, `clipboard.read`, and `clipboard.write`. |
