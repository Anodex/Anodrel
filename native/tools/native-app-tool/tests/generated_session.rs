...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path = temporary.join(format!(
            "anodrel-native-app-session-test-{}-{sequence}",
            std::process::id()
        ));
        fs::create_dir(&path).expect("create unique native-app session test directory");
        Self { path }
    }
}
...
impl Drop for TestDirectory {
    fn drop(&mut self) {
        let temporary = std::env::temp_dir();
        let expected_prefix = format!("anodrel-native-app-session-test-{}-", std::process::id());
