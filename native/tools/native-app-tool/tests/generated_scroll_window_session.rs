//! End-to-end proof for a project created by the native scroll-window generator.
...
use anodrel_ui::{ElementId, UiEvent, UiNode};
...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path = temporary.join(format!(
            "anodrel-native-scroll-window-app-session-test-{}-{sequence}",
            std::process::id()
        ));
        fs::create_dir(&path).expect("create unique native scroll-window test directory");
        Self { path }
    }
}
...
impl Drop for TestDirectory {
    fn drop(&mut self) {
        ...
        let expected_prefix = format!(
            "anodrel-native-scroll-window-app-session-test-{}-",
            std::process::id()
        );
...
#[test]
fn generated_project_completes_the_fixed_authenticated_scroll_window_walkthrough() {
    let temporary = TestDirectory::new();
    let destination = temporary.path.join("generated-scroll-window-session-app");
    let generated = Command::new(env!("CARGO_BIN_EXE_anodrel-native-app-tool"))
        .args([
            "init-scroll-window",
            destination.to_str().expect("test destination is UTF-8"),
            "generated-scroll-window-session-app",
            "Generated Scroll Window Session App",
        ])
        .output()
        .expect("run native scroll-window application generator");
    assert!(
        generated.status.success(),
        "the generator must accept valid scroll-window input"
    );
    ...
    let built = Command::new(env!("CARGO"))
        ...
        .status()
        .expect("build generated native scroll-window project");
    assert!(
        built.success(),
        "generated native scroll-window project must build"
    );
    let executable = target_directory
        .join("release")
        .join("generated-scroll-window-session-app.exe");
    assert!(
        executable.is_file(),
        "release build must produce the generated scroll-window executable"
    );
    ...
    .expect("fixed scroll-window template policy is valid");
    let (server, invitation) =
        ...
        .expect("create fixed scroll-window template test pipe");
    let bootstrap = invitation
        .bootstrap_invitation()
        .expect("convert scroll-window invitation to bootstrap record");
    ...
    let child = launch(&command, &bootstrap).expect("launch generated scroll-window child");
    ...
    assert!(
        ...
        "the simulated host commits the v2 secondary after native registration"
    );
    ...
    assert!(matches!(
        secondary_initial.document().root(),
        UiNode::Scroll(_)
    ));
    secondary_resources
        .input_mailbox()
        .push(UiInputCandidate::new(
            ...
            UiEvent::ActionInvoked(
                ElementId::new(SECONDARY_REVEAL_ACTION)
                    .expect("fixed secondary reveal action is valid"),
            ),
        ));
    ...
    assert!(matches!(
        secondary_updated.document().root(),
        UiNode::Scroll(_)
    ));
    secondary_resources
        .input_mailbox()
    ...
    assert!(
        ...
        "the simulated native destroy releases only the exact v2 secondary"
    );
    assert!(
        close_signal.take(),
    ...
    assert_eq!(
        child
            ...
            .expect("generated scroll-window template exits within its bound"),
        0,
        "generated scroll-window template must complete each fixed stage"
    );
    worker
        .join()
        .expect("scroll-window template pipe worker does not panic")
        .expect("scroll-window template pipe worker completes");
}
...
fn wait_for_document(
    ...
    panic!("generated scroll-window template did not deliver a document within its bound");
}
...
fn wait_for_open(
    ...
    panic!("generated scroll-window template did not request a secondary within its bound");
}
...
fn wait_for_secondary_close(
    ...
    panic!("generated scroll-window template did not request secondary close within its bound");
}

fn assert_child_is_running(child: &LaunchedProcess, stage: &str) {
    if let Ok(exit_code) = child.wait_for_exit(0) {
        panic!("generated scroll-window template stopped at safe stage {exit_code} before {stage}");
    }
}
