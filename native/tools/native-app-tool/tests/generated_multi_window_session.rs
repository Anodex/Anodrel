...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path = temporary.join(format!(
            "anodrel-native-multi-window-app-session-test-{}-{sequence}",
            std::process::id()
        ));
        fs::create_dir(&path).expect("create unique native multi-window test directory");
        Self { path }
    }
}
...
impl Drop for TestDirectory {
    fn drop(&mut self) {
        ...
        let expected_prefix = format!(
            "anodrel-native-multi-window-app-session-test-{}-",
            std::process::id()
        );
