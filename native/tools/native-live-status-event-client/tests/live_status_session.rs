//! End-to-end proof for the compiled changed-live-status diagnostic.
...
const PREPARE_ACTION: &str = "native.live.prepare";
const COMPLETE_ACTION: &str = "native.live.complete";
const CHILD_TIMEOUT_MILLISECONDS: u32 = 30_000;
const DOCUMENT_WAIT: Duration = Duration::from_secs(20);
const POLL_INTERVAL: Duration = Duration::from_millis(20);

#[test]
fn the_compiled_child_completes_one_authenticated_changed_live_status() {
    ...
    let command = BootstrapCommand::new(env!(
        "CARGO_BIN_EXE_anodrel-native-live-status-event-client"
    ))
    .expect("live-status diagnostic executable path is valid");
    ...
    assert_eq!(
        initial.document().status().map(|status| status.value()),
        Some("Ready to publish a visible result.")
    );
    ...
    assert_eq!(
        replacement.document().status().map(|status| status.value()),
        Some("Verification requires attention.")
    );
    input_mailbox.push(UiInputCandidate::new(
