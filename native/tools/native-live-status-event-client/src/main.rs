//! A compiled diagnostic for one authenticated changed live status.
//!
//! The child owns two strict version-3 documents and two semantic actions. It
//! publishes a polite status, accepts one host-selected action, then replaces
//! it with a changed assertive status under the same semantic ID. It accepts no
//! arguments, configuration, files, network input, or event data.
...
type LiveStatusEventClient = Client<WindowsClientStream>;
...
fn run() -> Stage {
    let Ok(invitation) = LiveStatusEventClient::read_invitation(&mut io::stdin()) else {
    ...
    let Ok(client) = LiveStatusEventClient::authenticate(stream, invitation) else {
        return Stage::AuthenticationRejected;
    };
    let mut session = UiSession::new(client);

    if session
        .replace_document_v3(INITIAL_DOCUMENT)
    ...
    if session
        .replace_document_v3(REPLACEMENT_DOCUMENT)
