...
fn stage_no_restart(package_root: &str) -> Result<String, String> {
    let root = std::path::Path::new(package_root);
    package::stage_no_restart(root).map_err(|_| {
        "the no-restart fixture package could not be staged; check the target directory".to_owned()
    })?;
    Ok(format!(
...
mod tests {
    ...
    fn usage_names_every_supported_command_and_nothing_else() {
        ...
        assert!(!USAGE.contains("provision-no-restart"));
    }
