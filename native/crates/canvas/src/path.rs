...
mod tests {
    ...
    fn inset_moves_a_square_inward_on_every_side() {
        let inner = square().inset(2.0);
        assert_eq!(inner.bounds(), Rect::new(2.0, 2.0, 8.0, 8.0));
    }
    ...
    fn inset_is_independent_of_winding_direction() {
        ...
        reversed.reverse();
        let inner = Path::polygon(reversed).inset(2.0);
        assert_eq!(inner.bounds(), Rect::new(2.0, 2.0, 8.0, 8.0));
    }
    ...
    fn bevel_bands_cover_every_edge_with_outward_normals() {
        ...
        // The top edge runs left to right, so its outward normal points up.
        let top = &bands[0];
        assert!((top.outward_normal.x).abs() < 1e-5);
        assert!(top.outward_normal.y < 0.0);
    }
    ...
    fn fit_unit_square_places_normalised_geometry() {
        ...
        let placed = unit.fit_unit_square(Rect::new(10.0, 20.0, 110.0, 220.0));
        assert_eq!(placed.bounds(), Rect::new(10.0, 20.0, 110.0, 220.0));
    }
}
