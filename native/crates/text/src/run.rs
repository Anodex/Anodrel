...
pub const MAX_RUN_ADVANCE_DESIGN_UNITS: u32 = 1_048_576;
...
impl TextRun {
    ...
    /// that glyph's horizontal advance. It deliberately does not perform
    /// fallback, kerning, ligatures, shaping, or line breaking.
    pub fn build(face: &FontFace<'_>, text: &str) -> Result<Self, TextRunError> {
        ...
        for character in text.chars() {
            ...
            let next_advance = advance_width
                .checked_add(u32::from(metric.advance_width()))
                .filter(|value| *value <= MAX_RUN_ADVANCE_DESIGN_UNITS)
                .ok_or(TextRunError::AdvanceLimitExceeded)?;
            glyphs.push(RunGlyph {
                glyph,
                pen_x: advance_width,
                advance_width: metric.advance_width(),
            });
            advance_width = next_advance;
        }
    ...
    pub const fn advance_width(&self) -> u32 {
        self.advance_width
    }
}
