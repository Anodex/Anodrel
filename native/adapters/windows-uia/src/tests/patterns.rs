...
fn an_enabled_authenticated_button_exposes_and_queues_only_invoke() {
    ...
    unsafe {
        ...
        assert_eq!(
            pattern, simple,
            "patterns retain the provider's COM identity"
        );
        ...
        let vtable = *queried.cast::<*const InvokeVtbl>();
        assert_eq!(((*vtable).invoke)(queried), S_OK);
...
fn a_field_exposes_its_value_without_an_automation_write() {
    ...
    unsafe {
        ...
        let vtable = *queried.cast::<*const ValueVtbl>();
        let mut value = ptr::null_mut();
        assert_eq!(((*vtable).get_value)(queried, &mut value), S_OK);
        assert!(!value.is_null());
        assert_eq!(crate::raw::copy_and_free_bstr(value), "Ada");

        let mut read_only = 0;
        assert_eq!(((*vtable).get_is_read_only)(queried, &mut read_only), S_OK);
        assert_eq!(read_only, 1);
        assert_eq!(
            ((*vtable).set_value)(queried, ptr::null()),
            UIA_E_NOTSUPPORTED
        );
        release_provider(provider);
        release_provider(provider);
...
fn only_the_selected_overflowing_group_exposes_standard_vertical_scroll() {
    ...
    unsafe {
        ...
        let vtable = *queried.cast::<*const ScrollVtbl>();
        ...
        assert_eq!(
            ((*vtable).get_horizontal_scroll_percent)(queried, &mut horizontal),
            S_OK
        );
        assert_eq!(
            ((*vtable).get_vertical_scroll_percent)(queried, &mut vertical),
            S_OK
        );
        assert_eq!(((*vtable).get_vertical_view_size)(queried, &mut view), S_OK);
        assert_eq!(
            ((*vtable).get_horizontally_scrollable)(queried, &mut horizontal_enabled),
            S_OK
        );
        assert_eq!(
            ((*vtable).get_vertically_scrollable)(queried, &mut vertical_enabled),
            S_OK
        );
        ...
        assert_eq!(((*vtable).scroll)(queried, 2, 4), S_OK);
        assert_eq!(((*vtable).set_scroll_percent)(queried, -1.0, 37.5), S_OK);
        assert_eq!(((*vtable).scroll)(queried, 4, 4), UIA_E_NOTSUPPORTED);
        assert_eq!(
            ((*vtable).set_scroll_percent)(queried, -1.0, f64::NAN),
            UIA_E_NOTSUPPORTED
        );
        release_provider(provider);
        release_provider(provider);
...
fn an_offscreen_scroll_descendant_exposes_only_scroll_item() {
    ...
    unsafe {
        ...
        let vtable = *queried.cast::<*const ScrollItemVtbl>();
        assert_eq!(((*vtable).scroll_into_view)(queried), S_OK);
        release_provider(provider);
        release_provider(provider);
        release_provider(provider);
    }
