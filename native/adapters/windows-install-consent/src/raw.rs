//! Narrow User32 confirmation dialog for one signed initial installation.
...
const INSTALL_CAPTION: &str = "Anodrel installer";

/// One direct fixed confirmation outcome from Windows.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum NativeConsent {
    /// The person approved a later fixed UAC route.
    Approved,
    /// The person declined without entering the UAC route.
    Declined,
}
...
/// Shows the only supported initial-install confirmation on its UI thread.
pub(super) fn request(version: PackageVersion) -> Result<NativeConsent, ()> {
    let text = wide(&dialog_text(version));
    let caption = wide(INSTALL_CAPTION);
    ...
    // slice; callers must invoke it from their native UI thread.
    let result = unsafe {
        MessageBoxW(
            std::ptr::null_mut(),
            text.as_ptr(),
...
fn dialog_text(version: PackageVersion) -> String {
    format!(
        "Install Anodrel version {}.{}.{} for all users?",
        version.major(),
        version.minor(),
        version.patch()
    )
...
mod tests {
    ...
    fn dialog_shows_only_the_signed_candidate_version_and_defaults_to_no() {
        assert_eq!(
            ...
            "Install Anodrel version 2.14.7 for all users?"
        );
